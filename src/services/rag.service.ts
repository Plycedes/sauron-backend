...
export async function query(input: RagQueryInput, requestingUserId: string): Promise<RagQueryResult> {
    ...
    const results = await (mongoose.connection.db!.collection("embeddings")).aggregate(pipeline).toArray();
