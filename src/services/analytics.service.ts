...
import { DailyUpdateModel, ProjectModel, UserModel } from "../models/mongo";
...
async function assertCompanyAccess(companyId: string, requestingUserId: string): Promise<void> {
...
export async function getProjectStats(
    ...
    return {
        ...
        categoryBreakdown: overallData ? categoryCounts(overallData.categories) : { ...ZERO_CATEGORY_BREAKDOWN },
        dateRange: {
...
export async function getUserStats(
    ...
    if (!target.companyId || requester.companyId?.toString() !== target.companyId.toString()) {
        throw new ApiError(403, "You can only view stats for users in your own company");
    }
